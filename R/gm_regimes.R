# optimfunct_regime <- function (rhopar, v, verbose = FALSE){
...
# }
#
# optimfunct_eff_regime <- function (rhopar, v, vcmat, verbose = FALSE){
#   vv <- v$bigG %*% c(rhopar[1], rhopar[1]^2) - v$litg
#   value <- t(vv) %*% vcmat %*% vv
#   if (verbose) cat("function:", value, "lambda:", rhopar[1], "\n")
#   value
...
# }
#
# psirhorho_hom_regime <- function(rho, residuals, Hmat, Zmat, Ws, d, v.vec){
#   n <- length(residuals)
...
# }
#
# psirhorho_het_regime <-function(rho, residuals, Hmat, Zmat, Ws, step1.c){
#   n <- length(residuals)
#
...
# }
#
# Omega_hom_regime <- function(rho, Pmat, A1, A2, a.vec1, a.vec2, Hmat, bigG, Phirri, epsilon, Zstar){
#   n <- length(epsilon)
#   Jota <- bigG %*% c(1,2*rho)
#   mu3 <- sum(epsilon^3) / n
#   sigma2 <- crossprod(epsilon)/n
...
# }
#
# Omega_het_regime <- function(rho, Pmat, A1, A2, a.vec1, a.vec2, Hmat, bigG, Phirri, epsilon, Zstar, Ws, step1.c){
#   n <- length(epsilon)
#   gamma <- epsilon^2
